...
class UserService
{
    ...
    public function getPlayerProfile ($user_slug){
        $user = User::connect(config('database.secondary'))
           ...
           // ->addSelect(DB::raw('IF((SELECT name as nationality FROM nationalities WHERE id = users.nationality_id ) IS NULL,NULL,(SELECT name FROM countries WHERE id = users.country_id )) as country'))
            ->first();
